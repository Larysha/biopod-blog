...
If you want to undo a commit but keep the history (safer than `reset --hard`):
...
You're working on something but need to switch branches without committing incomplete work:

```bash
git stash
```

Your changes are saved temporarily. Switch branches, do other work, then come back:
...
This restores your saved changes.

### Resetting to a Specific Commit

**Warning: This rewrites history. Only do this on commits that haven't been pushed.**
...
Everything after that commit is deleted. If you've already pushed these commits to GitHub and want to force the remote to match:

```bash
git push --force origin main
```

**Use this cautiously.** Force pushing can cause problems for collaborators.

### Combining Reset and Stash

Want to reset your git history but keep your current working changes?

```bash
git stash                    # Save current changes
git reset --hard a3f2b1c     # Reset to old commit
...
git reset --hard a3f2b1c  # Reset to specific commit (destructive)
git push --force           # Force push (use cautiously)
```
...
- If needed, add details after a blank line
- Be specific: "fix off-by-one error in loop" not "fix bug"
...
**Advanced branching:** Rebase, cherry-pick, interactive rebase
